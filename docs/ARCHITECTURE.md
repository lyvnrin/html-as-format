...
In all three cases the output is a single, complete HTML file with no external dependency at view time beyond the CDN-hosted `html2pdf.js`.
...
All three templates bake in the same chrome: a dark/light mode toggle, and a Download PDF button built on `html2pdf.js`. The PDF button is currently wired up but disabled in the UI, kept in place for when that feature is re-enabled. Each template ships one fixed accent colour that mirrors the generator app's own palette; there is no runtime colour picker in the generated output.
