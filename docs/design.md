...
Parses the source document (PPT, PDF, transcript) once and outputs a clean, structured intermediate format — slide headings, content per section, key points, metadata. Format-agnostic and reusable regardless of what view the user picks.
...
- Bubble Map - an interactive, clustered map for non-linear exploration
