...
- Both templates share an identical "chrome" (theme tokens, toolbar CSS/HTML, dark mode + theme-switcher + PDF-export JS, and a global hide-scrollbars rule — `* { scrollbar-width: none; -ms-overflow-style: none; } *::-webkit-scrollbar { display: none; }` right after the box-sizing reset). If you build a third format later, copy that shared chrome verbatim from either template rather than rewriting it, so all formats stay visually and behaviorally consistent. Scrolling must still work everywhere — this only hides the visible track/thumb.
